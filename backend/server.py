...
def _compile_spec(spec: Dict[str, Any]) -> str:
    ...
    try:
        ...
        compile_asl(str(temp_path), output_dir=str(output_dir))
        output_path = output_dir / f"compiled_{temp_path.stem}.py"
        if not output_path.exists():
            raise FileNotFoundError("Compiled output not found")
